...
class TestAuthRoutes:
    ...
    def test_init_auth_service(self):
        ...
        with patch("ciris_manager.api.auth_routes.GoogleOAuthProvider") as mock_provider:
            with patch("ciris_manager.api.auth_routes.InMemorySessionStore"):
                with patch("ciris_manager.api.auth_routes.SQLiteUserStore"):
                    ...
                    mock_provider.assert_called_once_with(
                        client_id="test-id", client_secret="test-secret", hd_domain="ciris.ai"
                    )
    ...
    def test_init_auth_service_from_env(self):
        ...
        with patch.dict(
            "os.environ", {"GOOGLE_CLIENT_ID": "env-id", "GOOGLE_CLIENT_SECRET": "env-secret"}
        ):
    ...
    def test_oauth_callback_success(self, client, mock_auth_service):
        ...
        mock_auth_service.handle_oauth_callback.assert_called_once_with("test-code", "test-state")
    ...
    def test_get_current_user_authenticated(self, client, mock_auth_service):
        ...
        mock_auth_service.get_current_user.return_value = {"user_id": 1, "email": "test@ciris.ai"}

        response = client.get("/v1/oauth/user", headers={"Authorization": "Bearer test-token"})
    ...
    def test_get_current_user_not_authenticated(self, client, mock_auth_service):
        ...
        response = client.get("/v1/oauth/user", headers={"Authorization": "Bearer invalid-token"})
