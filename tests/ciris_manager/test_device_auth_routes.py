...
class TestDeviceAuthRoutes:
    ...
    def test_request_device_code(self, client):
        ...
        response = client.post("/device/code", json={
            "client_id": "ciris-cli",
            "scope": "manager:full"
        })
    ...
    def test_poll_token_pending(self, client):
        ...
        # Poll for token
        response = client.post("/device/token", json={
    ...
    def test_verify_device_code_not_authenticated(self, client, auth_service):
        ...
        response = client.post("/device/verify", json={
            "user_code": "TEST-CODE"
        })
    ...
    def test_verify_device_code_invalid(self, client):
        ...
        response = client.post("/device/verify", 
    ...
    def test_complete_flow(self, client, auth_service):
        ...
        verify_response = client.post("/device/verify",
        ...
        # 3. Poll for token
        token_response = client.post("/device/token", json={
            "device_code": device_code,
            "client_id": "ciris-cli"
        })
