...
class TestJailbreakerIntegration:
    ...
    async def test_full_oauth_flow(self, client, jailbreaker_service):
        ...
        with patch.object(jailbreaker_service.discord_client, 'exchange_code_for_token') as mock_exchange:
            with patch.object(jailbreaker_service.discord_client, 'verify_jailbreak_permission') as mock_verify:
                ...
                callback_response = client.post("/jailbreaker/oauth/callback", json={
                ...
                reset_response = client.post("/jailbreaker/reset", json={
    ...
    async def test_rate_limiting_flow(self, client, jailbreaker_service, temp_dir):
        ...
        with patch.object(jailbreaker_service.discord_client, 'verify_jailbreak_permission') as mock_verify:
            ...
            reset_response1 = client.post("/jailbreaker/reset", json={
            ...
            reset_response2 = client.post("/jailbreaker/reset", json={
                "access_token": "test_token"
            })
    ...
    async def test_status_endpoint_with_user(self, client, jailbreaker_service):
        ...
        with patch.object(jailbreaker_service.discord_client, 'verify_jailbreak_permission') as mock_verify:
            ...
            mock_user = DiscordUser(id="status_test_user", username="statustest", discriminator="0001")
            mock_verify.return_value = (True, mock_user)
