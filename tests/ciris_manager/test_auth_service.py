...
class TestSQLiteUserStore:
    ...
    def test_create_user(self, user_store):
        ...
        user_id = user_store.create_or_update_user(
            "test@ciris.ai", {"name": "Test User", "picture": "http://example.com/pic.jpg"}
        )
    ...
    def test_update_existing_user(self, user_store):
        ...
        # Create user
        user_id1 = user_store.create_or_update_user("test@ciris.ai", {"name": "Original Name"})

        # Update user
        user_id2 = user_store.create_or_update_user("test@ciris.ai", {"name": "Updated Name"})
    ...
    def test_is_user_authorized(self, user_store):
        ...
        assert user_store.is_user_authorized("authorized@ciris.ai")
...
class TestAuthService:
    ...
    async def test_initiate_oauth_flow(self, auth_service, mock_oauth_provider):
        ...
        state, auth_url = await auth_service.initiate_oauth_flow(
            redirect_uri="http://app.example.com", callback_url="http://app.example.com/callback"
        )
    ...
    def test_create_jwt_token(self, auth_service):
        ...
        payload = {"user_id": 1, "email": "test@ciris.ai"}
    ...
    def test_get_current_user_valid(self, auth_service):
        ...
        # Create token
        token = auth_service.create_jwt_token({"user_id": 1, "email": "test@ciris.ai"})
    ...
    def test_get_current_user_unauthorized_email(self, auth_service):
        ...
        token = auth_service.create_jwt_token({"user_id": 999, "email": "nonexistent@ciris.ai"})
