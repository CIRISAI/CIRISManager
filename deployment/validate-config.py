...
class ConfigValidator:
    ...
    def _validate_auth_config(self, config: Dict[str, Any]):
        ...
        if mode != 'production':
            self.errors.append(f"Auth mode must be 'production' for production deployment, got: {mode}")
    ...
    def _validate_environment(self):
        ...
        if stat.st_mode & 0o077:
            self.warnings.append(f"Environment file has insecure permissions: {oct(stat.st_mode)[-3:]}")
