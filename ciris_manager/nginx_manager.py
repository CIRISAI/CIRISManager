...
This module handles dynamic nginx configuration generation for agent routes.
It generates partial configuration files (agents.conf) that are included
by the main nginx configuration.
"""
...
class NginxManager:
    ...
    def __init__(
        self, config_dir: str = "/opt/ciris-manager/nginx", container_name: str = "ciris-nginx"
    ):
        """
        ...
        self.config_path = self.config_dir / "agents.conf"
        self.new_config_path = self.config_dir / "agents.conf.new"
        self.backup_path = self.config_dir / "agents.conf.backup"

        # Ensure directory exists
        self.config_dir.mkdir(parents=True, exist_ok=True)

        # Ensure agents.conf exists (nginx fails if include file is missing)
        if not self.config_path.exists():
            logger.info(f"Creating initial empty agents.conf at {self.config_path}")
            self.config_path.write_text(
                "# Generated by CIRISManager - DO NOT EDIT\n"
                "# This file is included inside the main server { } block\n"
                "# Dynamic agent routes only - static routes are in the main config\n\n"
    ...
    def generate_config(self, agents: List[Dict]) -> str:
        """
        Generate partial nginx configuration for agent routes only.

        Args:
            agents: List of agent dictionaries

        Returns:
            Agent routes configuration to be included in main nginx.conf
        """
        return self._generate_agent_routes(agents)

    def _generate_agent_routes(self, agents: List[Dict]) -> str:
        """
        Generate agent upstreams and route locations for nginx include.

        Args:
            agents: List of agent dictionaries

        Returns:
            Nginx upstreams and location blocks for agents
        """
        config = "# Generated by CIRISManager - DO NOT EDIT\n"
        config += "# This file is included inside the main server { } block\n"
        config += "# Dynamic agent routes only - static routes are in the main config\n\n"

        # Add dynamic agent upstreams and routes
        if agents:
            config += "# Dynamic agent upstreams and routes\n"
            ...
            for agent in agents:
                ...
                # Agent upstream
                config += f"""upstream agent_{agent_id} {{
...
"""
                ...
                # Agent route
                config += f"""location ~ ^/api/{agent_id}/(.*)$ {{
...
"""
        ...
        # Add default /v1 route (typically points to 'datum' agent)
        default_agent = None
        if agents:
            # Look for 'datum' agent first, otherwise use first agent
            default_agent = next(
                (a for a in agents if a.get("agent_id", a.get("id")) == "datum"), agents[0]
            )

        if default_agent:
            agent_id = default_agent.get("agent_id", default_agent.get("id"))
            config += f"""# Default API route (points to {agent_id})
location /v1/ {{
