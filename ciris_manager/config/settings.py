...
class PortConfig(BaseModel):
    ...
    reserved: List[int] = Field(
        default=[8888, 3000, 80, 443], description="Ports to never allocate"
    )
...
class DockerConfig(BaseModel):
    ...
    registry: str = Field(default="ghcr.io/cirisai", description="Docker registry for images")
    image: str = Field(default="ciris-agent:latest", description="Default agent image")
...
class ManagerConfig(BaseModel):
    ...
    socket: Optional[str] = Field(
        default="/var/run/ciris-manager.sock", description="Unix socket path"
    )
    host: str = Field(default="0.0.0.0", description="API host")
    agents_directory: str = Field(
        default="/etc/ciris-manager/agents", description="Directory for agent configurations"
    )
