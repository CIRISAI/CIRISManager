...
class DiscordAuthClient:
    ...
    async def exchange_code_for_token(self, code: str) -> str:
        ...
        response = await self._client.post(
            f"{self.DISCORD_OAUTH_BASE}/token",
            data=token_data,
            headers=headers
        )
    ...
    async def get_guild_member(self, access_token: str, user_id: str) -> Optional[DiscordGuildMember]:
        """
        ...
        try:
            ...
            if e.response.status_code == 404:
                logger.debug(f"User {user_id} is not a member of guild {self.config.discord_guild_id}")
                return None
            raise
    ...
    async def verify_jailbreak_permission(self, access_token: str) -> tuple[bool, Optional[DiscordUser]]:
        """
