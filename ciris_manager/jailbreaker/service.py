...
class JailbreakerService:
    ...
    def __init__(self, config: JailbreakerConfig, agent_dir: Path, container_manager):
        ...
        logger.info(f"Rate limits: global={config.global_rate_limit}, user={config.user_rate_limit}")
    ...
    async def verify_access_token(self, access_token: str) -> tuple[bool, Optional[str]]:
        ...
        """
        try:
            has_permission, user = await self.discord_client.verify_jailbreak_permission(access_token)
            if user:
                ...
                try:
                    async with httpx.AsyncClient(timeout=5.0) as client:
                        ...
                        if response.status_code == 200:
                            logger.debug(f"Agent {self.config.target_agent_id} healthy on port {port}")
                            return True
    ...
    async def reset_agent(self, access_token: str) -> ResetResult:
        ...
        try:
            ...
            if self.config.agent_service_token:
                ...
                if not is_healthy:
                    logger.warning(f"Agent {self.config.target_agent_id} may not have started correctly")
                else:
