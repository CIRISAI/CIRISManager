...
class RateLimiter:
        ...
        async with self._lock:
            ...
            if self._last_global_reset is not None:
                ...
                if time_since_global < self.global_limit_seconds:
                    ...
                    logger.debug(f"Global rate limit hit, {self.global_limit_seconds - time_since_global:.1f}s remaining")
            ...
            if user_id in self._user_resets:
                ...
                if time_since_user < self.user_limit_seconds:
                    ...
                    logger.debug(f"User {user_id} rate limit hit, {self.user_limit_seconds - time_since_user:.1f}s remaining")
                    return False, next_allowed
    ...
    async def record_reset(self, user_id: str) -> None:
        ...
        async with self._lock:
            ...
            logger.info(f"Recorded reset for user {user_id} at {datetime.fromtimestamp(current_time)}")
            
    async def cleanup_old_entries(self) -> None:
        """Clean up old user reset entries to prevent memory bloat."""
        async with self._lock:
            current_time = time.time()
            cutoff_time = current_time - (self.user_limit_seconds * 2)  # Keep entries for 2x the limit
            
            old_users = [
                user_id for user_id, reset_time in self._user_resets.items()
