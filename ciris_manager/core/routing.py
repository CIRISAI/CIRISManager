...
def _validate_container(container, result: Dict[str, Any]) -> None:
    ...
    if not has_port:
        result["skipped"].append(
            {"name": container.name, "reason": "No accessible port", "agent_id": agent_id}
        )
    else:
    ...
    try:
        ...
        for container in containers:
            try:
                ...
                result["errors"].append({"name": container.name, "error": str(e)})

    except Exception as e:
        logger.error(f"Error connecting to Docker: {e}")
        result["errors"].append({"name": "docker", "error": str(e)})
